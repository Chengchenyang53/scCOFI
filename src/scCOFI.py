...
class scMultiCluster(nn.Module):
    def __init__(self, input_dim1, input_dim2,
            ...
            activation="elu", sigma1=2.5, sigma2=.1, alpha=1., gamma=1., phi1=0.0001, phi2=0.0001, cutoff = 0.5, lamda = 1, temperature=0.8):
        ...
        self.device = device
        self.encoder1 = buildNetwork2([input_dim1]+encodeLayer, type="encode", activation=activation)
        self.encoder2 = buildNetwork2([input_dim2] + encodeLayer, type="encode", activation=activation)
        self.encoderz = buildNetwork2([input_dim1+input_dim2]+encodeLayer, type="encode", activation=activation)
        ...
        self.criterion = Loss(256, self.tem, device).to(device)
        self.TransformerEncoderLayer = nn.TransformerEncoderLayer(d_model=self.z_dim, nhead=1, dim_feedforward=128,
                                                                  batch_first=True)
    ...
    def forward(self, x1, x2):
        ...
        x1h = self.encoder1(x1)
        xh1 = self.decoder1(x1h)
        xmean1 = self.dec_mean1(xh1)
        xdisp1 = self.dec_disp1(xh1)
        xpi1 = self.dec_pi1(xh1)

        x2h = self.encoder2(x2)
        xh2 = self.decoder2(x2h)
        xmean2 = self.dec_mean2(xh2)
        xdisp2 = self.dec_disp2(xh2)
        xpi2 = self.dec_pi2(xh2)
        ...
        h0 = self.encoderz(x0)
        commonz, S = self.TransformerEncoderLayer(h0)
        num, lq = self.cal_latent(commonz)
        return commonz, S, x1h, x2h, num, lq, mean1, mean2, disp1, disp2, pi1, pi2, xmean1, xmean2, xdisp1, xdisp2, xpi1, xpi2
    ...
    def encodeBatch(self, X1, X2, batch_size=256):
        ...
        for batch_idx in range(num_batch):
            ...
            inputs1 = Variable(x1batch).float()
            inputs2 = Variable(x2batch).float()
            z,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_ = self.forwardAE(inputs1, inputs2)
    ...
    def pretrain_autoencoder(self, X1, X_raw1, sf1, X2, X_raw2, sf2, 
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (x1_batch, x_raw1_batch, sf1_batch, x2_batch, x_raw2_batch, sf2_batch) in enumerate(dataloader):
                ...
                zbatch, S, x1h, x2h, z_num, lqbatch, mean1_tensor, mean2_tensor, disp1_tensor, disp2_tensor, pi1_tensor, pi2_tensor,xmean1_tensor, xmean2_tensor, xdisp1_tensor, xdisp2_tensor, xpi1_tensor, xpi2_tensor = self.forwardAE(x1_tensor, x2_tensor)
                recon_loss1 = self.zinb_loss(x=x_raw1_tensor, mean=mean1_tensor, disp=disp1_tensor, pi=pi1_tensor,
                                             scale_factor=sf1_tensor) + self.zinb_loss(x=x_raw1_tensor,
                                                                                       mean=xmean1_tensor,
                                                                                       disp=xdisp1_tensor,
                                                                                       pi=xpi1_tensor,
                ...
                recon_loss2 = self.zinb_loss(x=x_raw2_tensor, mean=mean2_tensor, disp=disp2_tensor, pi=pi2_tensor,
                                             scale_factor=sf2_tensor) + self.zinb_loss(x=x_raw2_tensor,
                                                                                       mean=xmean2_tensor,
                                                                                       disp=xdisp2_tensor,
                                                                                       pi=xpi2_tensor,
                                                                                       scale_factor=sf2_tensor)

                contra_loss1 = self.criterion.Structure_guided_Contrastive_Loss(x1h, zbatch, S)
                contra_loss2 = self.criterion.Structure_guided_Contrastive_Loss(x2h, zbatch, S)
                ...
                if epoch+1 >= epochs * self.cutoff:
                   loss = recon_loss1 + recon_loss2 + (contra_loss1 + contra_loss2) * self.lamda + kl_loss * self.phi1
#                    loss = recon_loss1 + recon_loss2
                else:
                   loss = recon_loss1 + recon_loss2 + (contra_loss1 + contra_loss2) * self.lamda#+ kl_loss
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                ...
                contra_loss1_val += contra_loss1.item() * len(x1_batch)
                contra_loss2_val += contra_loss2.item() * len(x2_batch)
                if epoch+1 >= epochs * self.cutoff:
            ...
            contra_loss1_val = contra_loss1_val / X1.shape[0]
            contra_loss2_val = contra_loss2_val / X1.shape[0]
            kl_loss_val = kl_loss_val/num
            if epoch%self.t == 0:
               print('Pretrain epoch {}, Total loss:{:.6f}, ZINB loss1:{:.6f}, ZINB loss2:{:.6f},CONTRA loss1:{:.6f}, CONTRA loss2:{:.6f} KL loss:{:.6f}'.format(epoch+1, loss_val, recon_loss1_val, recon_loss2_val,contra_loss1_val,contra_loss2_val, kl_loss_val))
    ...
    def fit(self, X1, X_raw1, sf1, X2, X_raw2, sf2, y=None, lr=1., n_clusters = 4,
        ...
        for epoch in range(num_epochs):
            ...
            recon_loss1_val = 0.0
            recon_loss2_val = 0.0
            contra_loss1_val = 0.0
            contra_loss2_val = 0.0
            ...
            for batch_idx in range(num_batch):
                ...
                inputs1 = Variable(x1_batch).float()

                rawinputs1 = Variable(x_raw1_batch)
                sfinputs1 = Variable(sf1_batch)
                inputs2 = Variable(x2_batch).float()
                rawinputs2 = Variable(x_raw2_batch)
                sfinputs2 = Variable(sf2_batch)

                zbatch, S, x1h, x2h, z_num, lqbatch, mean1_tensor, mean2_tensor, disp1_tensor, disp2_tensor, pi1_tensor, pi2_tensor,xmean1_tensor, xmean2_tensor, xdisp1_tensor, xdisp2_tensor, xpi1_tensor, xpi2_tensor = self.forward(inputs1, inputs2)
                ...
                recon_loss1 = self.zinb_loss(x=rawinputs1, mean=mean1_tensor, disp=disp1_tensor, pi=pi1_tensor,
                                             scale_factor=sfinputs1) + self.zinb_loss(x=rawinputs1,
                                                                                       mean=xmean1_tensor,
                                                                                       disp=xdisp1_tensor,
                                                                                       pi=xpi1_tensor,
                ...
                recon_loss2 = self.zinb_loss(x=rawinputs2, mean=mean2_tensor, disp=disp2_tensor, pi=pi2_tensor,
                                             scale_factor=sfinputs2) + self.zinb_loss(x=rawinputs2,
                                                                                       mean=xmean2_tensor,
                                                                                       disp=xdisp2_tensor,
                                                                                       pi=xpi2_tensor,
                ...
                loss = recon_loss1 + recon_loss2 +contra_loss1_val+contra_loss2_val+ kl_loss * self.phi2 + cluster_loss * self.gamma
                optimizer.zero_grad()
                loss.backward()
#                torch.nn.utils.clip_grad_norm_(self.mu, 1)
                optimizer.step()
                cluster_loss_val += cluster_loss.data.cpu().item() * len(inputs1)
                recon_loss1_val += recon_loss1.data.cpu().item() * len(inputs1)
                recon_loss2_val += recon_loss2.data.cpu().item() * len(inputs2)
                contra_loss1_val += contra_loss1.item() * len(x1_batch)
                contra_loss2_val += contra_loss2.item() * len(x2_batch)
                kl_loss_val += kl_loss.data.item() * len(inputs1)
                loss_val += loss.data.item() * len(inputs1)
            num_samples = X1.shape[0]
            scheduler.step(loss_val / num_samples)

            loss_history.append(loss_val / num)
            if y is not None:
                ami_history.append(final_ami)
                nmi_history.append(final_nmi)
                ari_history.append(final_ari)

            if epoch % self.t == 0:
               print("#Epoch %d: Total: %.6f Clustering Loss: %.6f ZINB Loss1: %.6f ZINB Loss2: %.6f CONTRA Loss1: %.6f CONTRA Loss2: %.6fKL Loss: %.6f" % (
                     epoch + 1, loss_val / num, cluster_loss_val / num, recon_loss1_val / num, recon_loss2_val / num, contra_loss1_val / num, contra_loss2_val / num, kl_loss_val / num))
